---
name: gotcha-lookup
description: Use when debugging Salesforce errors, building or modifying Salesforce Flows (record-triggered, screen, autolaunched, scheduled), writing Apex triggers or invocable methods, or planning Salesforce metadata deployments. Cross-references a curated catalog of common Salesforce gotchas — FLS payload issues, namespace migration, Quick Action conventions, auto-number timing, recursion, bulkification, data-quality root causes, and more — before proposing fixes.
---
...
- A Salesforce deployment is being planned or has failed (metadata, fields, record types, flows)
- An error email arrived from a Flow / Process Builder / Apex
- The user is debugging "why did this not fire" / "why was this null" / "why did this not deploy"
- The user is asking about Flow best practices, governor limits, or context (system vs user mode)

## Workflow

1. **Identify the failure mode or feature area** from the user's message and any error text they shared.
2. **Load the matching reference module(s)** via Read:
   - `reference/flow-pitfalls.md` — flow runtime errors, screen flow quirks, fast-update FLS, auto-number timing, Quick Actions, subflow versioning
   - `reference/deployment-pitfalls.md` — metadata deploy gotchas, record type activation, permission set companions (coming in future versions)
   - `reference/tooling-api-pitfalls.md` — Tooling API quirks, SOQL on metadata, CustomObject table traps (coming in future versions)
...
4. **Cite matching gotcha numbers** when recommending fixes — gives the user a stable reference they can look up.
